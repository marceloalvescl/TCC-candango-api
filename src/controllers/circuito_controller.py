...
def userCompletedCircuit(requestJson):
    ...
    if verifyUserUnlockedCircuit(requestJson['circuitId']): 
        return {'error' : 'circuito já foi liberado pelo usuário'}
    elif userVisitedAllAttractionsOfCircuit:
...
def verifyUserUnlockedCircuit(circuitID):
    unlockedCircuit = UsuarioCircuito.query.filter(
        UsuarioCircuito.cod_circuito == circuitID,
        UsuarioCircuito.cod_usuario == current_user.id_usuario
    ).first()
    return unlockedCircuit
